...
const contactSchema = new mongoose.Schema({
    ...
    username: {
        ...
        minlength: [5,'name too short, please use a 5 to 50 characters contact name'],
        maxlength: [50,'name too long, please use a 5 to 50 characters contact name'],
    },
    email: {
        type: String,
...
contactSchema.pre('save',async function(next){
    const existent = await this.constructor.find({userId: this.userId, parentId: this.parentId});
    console.log(existent);
    if (existent.length === 0) {return next();}
    if (existent.length > 0) return next (new Error('failed to create duplicated contact'));
})

contactSchema.pre(/^find/, async function(next){
    this.populate( 'userId', '_id photo')
    next()
})
